...
Server &Server::operator=(Server const & other) { if (this != &other) { *this = other; } return (*this); }

Server::Server(uint16_t const & port, std::string & password) : _port(port), _password(password), _epollfd(-2)
{
...
void	Server::chooseCmd(t_commandArgs & cArgs)
{
	...
	*cArgs.sstream >> cmd;
	...
	{
		err_feedback = ERR_UNKNOWNCMD(cArgs.client->getNickname(), cmd);
